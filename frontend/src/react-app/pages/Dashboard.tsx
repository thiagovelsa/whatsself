...
export default function Dashboard() {
  ...
  // Refetch function that também atualiza o resumo
  ...
  const [clearedMessageIds] = useState<Set<string>>(new Set());
  void clearedMessageIds;

  // Auto-refetch critical data when WebSocket reconnects
  useEffect(() => {
  ...
  // Mensagens "limpas" em outras telas (ex.: /messages) também não devem aparecer aqui
  const mergedClearedMessageIds = useMemo(() => {
    return new Set<string>();
  }, []);
  void mergedClearedMessageIds;
  ...
  // Use aggregated metrics from backend (more efficient than loading full lists)
  ...
  // Chart data preparation (moved outside conditional to fix hooks rules)
  ...
  const todayMessagesHighlight =
    dashboardSummary?.metrics.todayMessages ?? todayMessages;
  ...
  // Get recent messages (last 5) diretamente das mensagens carregadas
  const availableMessages = messages || [];
  const recentMessages = availableMessages.slice(0, 5);

  // Handle clear all dashboard data
  const handleClearDashboard = async () => {
    // Clear React Query cache for dashboard-related queries
  ...
  const loading =
    statusLoading || messagesLoading || metricsLoading || summaryLoading;
    ...
    <div className="space-y-9">
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div className="flex w-full flex-wrap gap-3 lg:w-auto lg:justify-end">
            <button
              ...
              className="flex items-center gap-2 rounded-xl border border-brand-border/60 bg-brand-surface/80 px-4 py-2 text-sm font-semibold text-white transition hover:border-brand-primary/50 hover:text-brand-primary disabled:cursor-not-allowed disabled:opacity-60"
            >
            ...
            {isAdminUser && (
              <button
                ...
                className="flex items-center gap-2 rounded-xl border border-brand-border/60 bg-brand-surface/80 px-4 py-2 text-sm font-semibold text-white transition hover:border-amber-400/50 hover:text-amber-300 disabled:cursor-not-allowed disabled:opacity-60"
              >
            ...
            <button
              ...
              className="flex items-center gap-2 rounded-xl border border-brand-border/60 bg-brand-surface/80 px-4 py-2 text-sm font-semibold text-white transition hover:border-brand-primary/40 hover:text-brand-primary"
            >
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-3">
          {!dashboardSummary && summaryLoading
            ...
            : healthCards.map((card) => {
                const tone =
                  healthCardStyles[
                    card.status as keyof typeof healthCardStyles
                  ] || healthCardStyles.ok;
                return (
                  <div
                    key={card.id}
      ...
      <section className="rounded-3xl border border-brand-border/70 bg-brand-surfaceElevated/50 p-6 shadow-brand-soft">
        ...
        <div className="mt-6 space-y-4">
          {!dashboardSummary && summaryLoading ? (
            ...
            quickRecommendations.map((alert) => (
              <div
                key={alert.id}
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="space-y-1">
          ...
          <div className="flex items-center gap-3">
            <p className="text-sm text-brand-muted">
              Bem-vindo, {user?.name || "Operador"}! Aqui você acompanha em
              tempo real o status do WhatsApp, fila de mensagens e proteções do
              sistema.
            </p>
      ...
      {loading ? (
        <div className="grid gap-6">
          ...
          <div className="h-96 animate-pulse rounded-3xl border border-brand-border/50 bg-brand-surface/40" />
        </div>
      ) : (
        <>
          {/* Status & Realtime */}
          <section className="grid gap-6 xl:grid-cols-5">
            <div className="rounded-3xl border border-brand-border/70 bg-brand-surfaceElevated/60 p-8 shadow-brand-soft xl:col-span-3">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div className="flex-1">
                  ...
                  <p className="mt-1 text-sm text-brand-muted">
                    Status do WhatsApp, quantidade de mensagens aguardando envio
                    e sistema de proteção anti-ban.
                  </p>
                </div>
              ...
              <div className="mt-6 grid gap-4 md:grid-cols-2">
                <div className="rounded-2xl border border-brand-border/70 bg-brand-surface/70 p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p
                        className="text-xs uppercase tracking-wide text-brand-muted"
                        title="Mensagens aguardando para serem enviadas. O sistema processa automaticamente respeitando os limites de taxa."
                      >
                        Fila global
                      </p>
                      ...
                      <p className="mt-1 text-xs text-brand-muted">
                        {systemStatus?.queue?.processing
                          ? "Processando em tempo real"
                          : "Fila aguardando disponibilidade"}
                      </p>
                    </div>
                  ...
                  <div className="mt-4 h-2 rounded-full bg-brand-surface/90">
                    <div
                      ...
                      style={{
                        width: `${Math.min(100, queueSize ? Math.min(queueSize * 10, 100) : 12)}%`,
                      }}
                    />
                  </div>
                </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-brand-muted mt-1">
                        Conecte seu WhatsApp Business para começar a enviar e
              ...
              {!isBackendHealthy && (
                <div className="mt-6 rounded-2xl border border-rose-500/40 bg-rose-500/10 p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <p className="text-sm font-medium text-white">
                        Problema na conexão com o backend
                      </p>
                      <p className="text-xs text-brand-muted mt-1">
                        Não foi possível obter o status do sistema. Verifique se
            ...
            <div className="rounded-3xl border border-brand-border/70 bg-brand-surfaceElevated/60 p-8 shadow-brand-soft xl:col-span-2">
              ...
              <p className="mt-1 text-sm text-brand-muted">
                Limite de mensagens por minuto para simular comportamento humano
                e evitar bloqueios.
              </p>
              <div className="mt-6 space-y-4">
                <div>
                  ...
                  <div className="mt-2 h-2 rounded-full bg-brand-surface/90">
                    <div
                      ...
                      style={{
                        width: `${Math.min(100, Math.round((rateLimitUsage / rateLimitMax) * 100)) || 4}%`,
                      }}
                    />
                  </div>
                </div>
                ...
                <div className="rounded-2xl border border-brand-border/70 bg-brand-surface/70 p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-brand-muted">
                        {automationRate}% das mensagens foram automatizadas nas
                        últimas 24h. Quanto maior, mais eficiente está sua
                        automação.
                      </p>
                    </div>
                  </div>
          ...
          <section className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-4">
            <MetricCard
              ...
              change={
                automationRate > 50
                  ? "Automação no ritmo ideal"
                  : "Considere revisar os gatilhos"
              }
            ...
            <MetricCard
              ...
              description={
                failedMessages === 0
                  ...
                  : "Analise os casos com erro"
              }
              icon={<AlertCircle className="h-6 w-6" />}
              change={
                failedMessages === 0 ? "Operação estável" : "Requer atenção"
              }
              changeType={failedMessages === 0 ? "positive" : "negative"}
              href={failedMessages > 0 ? "/messages?status=failed" : undefined}
              tooltip={
                failedMessages > 0 ? "Ver mensagens com falha" : undefined
              }
            />
          </section>
            ...
            <div className="rounded-3xl border border-brand-border/70 bg-brand-surfaceElevated/60 shadow-brand-soft">
              <div className="flex items-center justify-between border-b border-brand-border/60 px-6 py-5">
                <div>
                  ...
                  <p className="text-sm text-brand-muted">
                    Últimas interações com seus contatos monitoradas em tempo
                    real.
                  </p>
                </div>
                <div className="flex items-center gap-3">
              ...
              {recentMessages.length === 0 ? (
                <div className="space-y-3 px-6 py-14 text-center text-brand-muted">
                  ...
                  <p className="text-sm">
                    Assim que clientes começarem a interagir, o histórico
                ...
                <div className="divide-y divide-brand-border/40">
                  {recentMessages.map((msg) => (
                    <div
                      key={msg.id}
                      className="flex flex-col gap-3 px-6 py-5 transition hover:bg-brand-surface/60"
                    >
                      <div className="flex flex-wrap items-center gap-3">
                        ...
                        <span className="text-xs text-brand-muted">
                          {msg.contact?.phone || "Contato não identificado"}
                        </span>
                      </div>
                ...
                <div className="mt-5 space-y-4">
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-xs text-brand-muted">
                        Histórico completo e envio manual
                      </p>
                    </div>
                  </Link>
                  <Link
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-xs text-brand-muted">
                        Segmentação e status de opt-in
                      </p>
                    </div>
                  </Link>
                  <Link
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-xs text-brand-muted">
                        Ajuste mensagens disparadas por gatilhos
                      </p>
                    </div>
                  </Link>
                </div>
              ...
              <div className="rounded-3xl border border-brand-border/70 bg-brand-surfaceElevated/60 p-6 shadow-brand-soft">
                ...
                <div className="mt-4 flex gap-4 text-xs text-brand-muted">
                  <div className="flex items-center gap-2">
                    ...
                    <span>
                      Total:{" "}
                      {timeseriesData.timeseries.reduce(
                        (sum, p) => sum + p.total,
                        0,
                      )}
                    </span>
              ...
              <div className="rounded-3xl border border-brand-border/70 bg-brand-surfaceElevated/60 p-6 shadow-brand-soft">
                ...
                <div className="mt-4 grid grid-cols-2 gap-2 text-xs text-brand-muted">
                  {timeseriesData.statusDistribution &&
                    Object.entries(timeseriesData.statusDistribution).map(
                      ([status, count]) => (
                        <div
                          key={status}
